...
def build_messages(model_name, task):
    
    if 'Chem_R' in model_name or 'Chem-R' in model_name:
        prompt = init_gen_Chem_R.retrieve_zero_prompts()
        _messages = [{"role": "system", "content": "You are an expert chemist."},
                    {"role": "user", "content": f"{prompt}{task}"}]
    elif 'gpt' in model_name.lower():
        prompt = init_gen_think.retrieve_zero_prompts_gpt()
        _messages = [{"role": "system", "content": prompt},
                    {"role": "user", "content": task}]
    else:
        prompt = init_gen_think.retrieve_zero_prompts()
        _messages = [{"role": "system", "content": prompt},
                    {"role": "user", "content": "\nInstruction: " + task + "\n"}]
    return _messages
...
def build_examiner_response(smiles, ignore_desc_list=None):
    ...
    for prop, value in props_dict.items():
        if value is None or prop not in prop_desc_map:
            continue
        elif ignore_desc_list and prop in ignore_desc_list: # avoid evaluation leakage
            continue
        else:
